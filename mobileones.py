...
from torch.nn.utils import parametrizations as param_utils
...
class MobileOne(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = int(512 * width_multipliers[3])
        # self.linear =  nn.Linear(self.embedding, num_classes)
        self.classifier = param_utils.weight_norm(
            nn.Linear(self.embedding, num_classes)
        )
...
class Model_Parrallel(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.head = mobileone(num_classes=21, variant="s4")
        self.end = mobileone(num_classes=21, variant="s4")

    def forward(self, x1, x2):

        x1, x2 = self.head.stage_list[0](x1), self.end.stage_list[0](x2)

        # x1, x2 = attention_femp_fuse(x1, x2)

        x1, x2 = self.head.stage_list[1](x1), self.end.stage_list[1](x2)

        # x1, x2 = attention_femp_fuse(x1, x2)

        x1, x2 = self.head.stage_list[2](x1), self.end.stage_list[2](x2)

        # x1, x2 = attention_femp_fuse(x1, x2)

        x1, x2 = self.head.stage_list[3](x1), self.end.stage_list[3](x2)

        # x1, x2 = attention_femp_fuse(x1, x2)

        x1, x2 = self.head.stage_list[4](x1), self.end.stage_list[4](x2)

        # x1, x2 = attention_femp_fuse(x1, x2)

        x1, x2 = self.head.stage_list[5](x1), self.end.stage_list[5](x2)

        x1, x2 = self.head.stage_list[6](x1), self.end.stage_list[6](x2)

        # x1, x2 = attention_emb_fuse(x1, x2)

        x1, x2 = self.head.stage_list[7](x1), self.end.stage_list[7](x2)
...
def reparameterize_model(model: torch.nn.Module, is_cls=True) -> nn.Module:
    ...
    # Remove weight normalization, copy the model, and then reapply weight normalization
    if is_cls:
        model.classifier.weight.detach_()
        model = copy.deepcopy(model)
        for module in model.modules():
            if hasattr(module, "reparameterize"):
                module.reparameterize()
        model.classifier.weight.requires_grad_()
    else:
        model = copy.deepcopy(model)
        for module in model.modules():
            if hasattr(module, "reparameterize"):
                module.reparameterize()
    # model.load_state_dict(weight_norm_layer, strict=False)
    return model
